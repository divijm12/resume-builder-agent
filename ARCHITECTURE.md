...
applications(
  ...
  match_score, resume_variant_path,
  resume_name,          -- which named resume (data/master_resumes/) produced this application --
                        -- a plain string snapshot (Stage -1's library), not a foreign key, so it
                        -- stays meaningful even if that resume is later renamed or deleted
  cover_letter_path,
...
**Out:** `{draft: {...full master-resume shape, with ids...}, validation_log, model, suggested_label}`.
**Never writes a resume file directly** -- `POST /api/master-resumes/parse`
returns the draft (plus the raw extracted text, so a garbled PDF
extraction is visible before it ever reaches the guardrail) for review;
only `POST /api/master-resumes/confirm` (a separate, explicit action,
given a human-chosen `label`) writes it, and only after backing up
whatever was there before at that same slug to
`data/master_resume_backups/<slug>_<timestamp>.yaml` (gitignored, same
trust tier as `applications.db`) -- a bad upload can never destroy an
existing resume with no way back. The frontend shows the draft as one
big editable textarea rather than a form with a field per schema key,
matching the "hand-editable text" pattern already used for outreach
drafts.

**A named library, not a single file (added 2026-09-01, explicit user
request):** `data/master_resumes/<slug>.yaml` holds as many resumes as
you want, side by side (e.g. different variants for different kinds of
roles) -- `data/master_resumes/_index.json` maps each slug to
`{label, updated_at}`. Uploading with a label matching an existing entry
refreshes that resume (backed up first, as above); a new label adds a
new one alongside the rest, never replacing it. `DELETE
/api/master-resumes/{slug}` removes one (backed up first too, and
refuses to delete the last resume in the library so the dashboard is
never left with nothing to select). The New Application page gets a
dropdown to pick which resume to score/tailor against
(`POST /api/jobs`'s `resume_slug` field, resolved server-side to a path +
label via the index); `applications.resume_name` records the human label
used (a plain string snapshot, not a foreign key, so it stays meaningful
even if that resume is later renamed or deleted) -- shown on both the
Applications list and Application Detail so it's never ambiguous which
resume produced a given application. Verified live end-to-end: created a
real application against a second real resume via the actual `/api/jobs`
endpoint, confirmed the DB row's `resume_name` and the rendered output
filename both reflected the correct person, not the default resume.
...
**Distinctive-detail guardrail, with a retry before reverting (added 2026-09-01):** the guardrails above only protect a number or a name already in the master resume's skills/tech vocabulary -- they don't protect other specific, meaningful detail that isn't on either list. The real, motivating case: bullet `b_004`'s "a turfgrass disease outbreak research project" reworded down to just "a research project" -- factually fine, no number or tech term changed, but the detail that makes the work distinctive and credible is gone. The prompt already explicitly bans this with this exact example (see `_SHARED_CORE_PROMPT`) and it still isn't reliable, which is why this moved to code, the same pattern as the appended-clause and dropped-tech-term guardrails before it. `_distinctive_phrases()` extracts runs of 3+ consecutive non-stopword words from a bullet's original text; `_dropped_phrases()` flags one if fewer than half its words survive (via whole-word matching) in the reworded version -- a majority threshold, not "any single word," specifically because a phrase like "turfgrass disease outbreak research project" bundles a specific detail with a generic trailing "research project" that would trivially "survive" on its own and mask the real loss. This is a structural heuristic with real false positives (it will also flag some harmless paraphrasing -- confirmed with a constructed example, "structured data quality validation" reworded to "rigorous data checks"), unlike the near-exact-match checks above, so it does **not** revert immediately: `validate_and_build` (now optionally accepting `client`/`retry_model` -- the one deliberate exception to it being a fully pure function) makes one targeted retry call asking the model to restore just the missing detail, re-runs the *entire* check suite (not just this one) against the retry's output, and only reverts to the original bullet if the retry still fails or introduces a new problem. Capped at `MAX_RETRIES_PER_RUN = 5` retries per call as a worst-case cost/latency safety net. Calling `validate_and_build` without a `client` (e.g. a direct unit test) skips the retry and reverts immediately, same as the other guardrails -- keeps existing simple test calls deterministic. Verified: a mocked turfgrass-style case is correctly flagged and, separately, correctly retried/accepted, retried/still-failed-so-reverted, and retried-fixed-one-thing-but-broke-another-so-reverted; a retroactive check against all 9 bullets genuinely reworded across the two real logged applications found zero false positives at the chosen threshold; two separate real pipeline runs (one direct, one through the actual dashboard) both correctly and faithfully preserved the turfgrass detail without ever needing to trigger a retry at all.
