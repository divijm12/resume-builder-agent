...
- [ ] Gmail API integration for send-after-approval only -- explicitly deferred, deliberate user choice to keep this pass draft-only; revisit as its own conversation
- [ ] Log outreach sent/response into `applications.db` -- no new mechanism needed when the time comes: `status` already includes `'outreach_sent'` as a manually-selectable value via the existing dropdown, and `outreach_sent_at`/`response_received` columns already exist unused, ready for a future manual "mark as sent" action

**Done when:** you can review and approve a full application + outreach package in under 3 minutes per company. Draft generation is done; sending remains a manual, deliberate action outside this tool for now.
...
**Current position (2026-08-31):** Phases 1 and 2 are both solid and well past their original bar. Phase 3 is logging/tracking-complete, its analytics item still deliberately waiting on more real usage (a handful of logged applications isn't enough volume for correlation queries to say anything meaningful). Phase 4's core contact-discovery flow, including the department-based relevance boost and named-in-JD targeted lookup, is now built and verified against two real companies (Anduril, Snowflake) -- worth testing against a few more before fully trusting it at volume. Phase 5's outreach draft agent is now built too, deliberately scoped to draft-only per explicit user choice -- no Gmail/OAuth/send capability exists anywhere in this codebase, so hard rule 2 ("never auto-send") is satisfied trivially rather than by careful gating. Actual sending (Gmail integration, a real send button) is the one remaining piece of Phase 5, and it's the first thing in this whole project that would touch a real, external, hard-to-undo action -- worth a deliberate conversation before starting, not a default next build.
